  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans" dir="rtl">
        ...
        <div className="max-w-6xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <nav className="flex gap-4 items-center text-sm font-medium">
            <button 
              ...
              className={`px-3 py-1.5 rounded-lg transition ${activeTab === 'home' ? 'bg-blue-50 text-blue-700' : 'text-slate-600 hover:text-blue-600'}`}
            >
              الرئيسية
            </button>
            <button 
              onClick={() => setActiveTab('admin')}
              className={`px-3 py-1.5 rounded-lg transition ${activeTab === 'admin' ? 'bg-blue-50 text-blue-700' : 'text-slate-600 hover:text-blue-600'}`}
            >
              لوحة الإدارة
            </button>
          </nav>
      ...
      {activeTab === 'home' ? (
              ...
              {articles.map((article) => (
                ...
                >
                  ...
                  <div className="pt-4 border-t border-slate-100 flex justify-between items-center text-xs text-slate-500">
                    ...
                    <span className="text-blue-600 font-semibold">قراءة المقال ←</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        /* لوحة الإدارة المبسطة الآمنة */
        <main className="max-w-xl mx-auto px-6 py-16">
          <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 text-center">
            <h3 className="text-xl font-bold text-slate-800 mb-2">لوحة الإدارة والتحكم</h3>
            <p className="text-sm text-slate-500 mb-6">إشراف د. هيثم الخطيب</p>
            {!isAdmin ? (
              <div className="space-y-4">
                <input 
                  type="password" 
                  placeholder="كلمة المرور (DrHaitham2026!)" 
                  value={password}
              ...
              <div className="bg-emerald-50 text-emerald-800 p-4 rounded-xl text-sm font-semibold">
                ✅ تم تسجيل الدخول بنجاح للوحة الإدارة. النظام يعمل بكفاءة عالية.
              </div>
            )}
          </div>
        </main>

...
const FAQS = [
  ...
  },
  {
    ...
    a: 'تُعامل جميع الاستشارات بخصوصية وسرية بين المريضة والطبيب وفق إجراءات التواصل المعتمدة. يمكنك التواصل عبر صفحة الاستشارات.',
  },
  {
    ...
    <div className="space-y-16 pb-16">
            ...
            <div className="lg:col-span-7 space-y-6 text-right">
              ...
              <div className="pt-4 grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs text-slate-600">
                <div className="flex items-center gap-2 bg-white/70 p-2.5 rounded-lg border border-slate-200">
                  ...
                  <span>خصوصية وسرية في التواصل</span>
                </div>
                <div className="flex items-center gap-2 bg-white/70 p-2.5 rounded-lg border border-slate-200">
            ...
            <div className="lg:col-span-5">
              <div className="bg-gradient-to-b from-white to-slate-50 p-6 sm:p-8 rounded-2xl border border-sky-200 shadow-2xl space-y-6">
                ...
                <div className="space-y-3 text-xs leading-relaxed text-slate-600">
                  <p>
                    تهدف منصة <strong>FemSeha</strong> إلى توفير التثقيف الصحي والمعلومات الطبية المبسطة في مجالات الحمل واضطرابات الدورة والخصوبة وسلامة الأدوية.
                  </p>
                </div>
                <a
                  href={WHATSAPP_LINK}
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-md hover:border-sky-600 transition-colors space-y-4 flex flex-col justify-between">
            <div className="space-y-3">
              <div className="w-12 h-12 rounded-xl bg-rose-50 border border-rose-200 flex items-center justify-center text-rose-400"><HeartIcon className="w-6 h-6" /></div>
              <h3 className="text-xl font-bold text-slate-900">الحمل والإجهاض والطوارئ</h3>
              <p className="text-xs text-slate-600 leading-relaxed">أعراض الحمل المبكر، مؤشرات الإجهاض التلقائي، النزيف في الحمل، وعلامات تستدعي التقييم الطبي العاجل.</p>
              <ul className="text-xs text-slate-500 space-y-1.5 pt-2 border-t border-slate-200">
                <li>• أعراض ومخاطر الحمل خارج الرحم</li>
                <li>• متى يكون النزيف أثناء الحمل طارئاً؟</li>
                <li>• العناية والتعافي بعد فقدان الحمل</li>
              </ul>
            </div>
            <Link to={CATEGORY_LINKS['دليل السلامة والطوارئ']} className="inline-flex items-center gap-1 text-blue-700 hover:text-blue-800 text-xs font-bold pt-4"><span>استكشف دليل السلامة والطوارئ</span><ChevronLeftIcon className="w-4 h-4" /></Link>
          </div>

          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-md hover:border-sky-600 transition-colors space-y-4 flex flex-col justify-between">
            <div className="space-y-3">
              <div className="w-12 h-12 rounded-xl bg-sky-50 border border-sky-200 flex items-center justify-center text-sky-400"><StethoscopeIcon className="w-6 h-6" /></div>
              <h3 className="text-xl font-bold text-slate-900">صحة المرأة وأعراض الحمل</h3>
              <p className="text-xs text-slate-600 leading-relaxed">علامات الحمل المبكرة، أسباب تأخر الدورة غير الحمل، وتكيس المبايض والخصوبة.</p>
              <ul className="text-xs text-slate-500 space-y-1.5 pt-2 border-t border-slate-200">
                <li>• أعراض الحمل الأولى</li>
                <li>• أسباب تأخر الدورة</li>
                <li>• تكيس المبايض والخصوبة</li>
              </ul>
            </div>
            <Link to={CATEGORY_LINKS['صحة المرأة']} className="inline-flex items-center gap-1 text-blue-700 hover:text-blue-800 text-xs font-bold pt-4"><span>تصفح قسم صحة المرأة</span><ChevronLeftIcon className="w-4 h-4" /></Link>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {featuredArticles.map((article) => (
            <article key={article.id} className="bg-white border border-slate-200 hover:border-sky-600 rounded-xl p-6 flex flex-col justify-between transition-all group">
              <div className="space-y-3">
                <div className="flex items-center justify-between text-[11px] text-slate-500">
                  <span className="bg-sky-50 text-sky-700 border border-sky-200 px-2.5 py-0.5 rounded-md font-semibold">{article.categoryName}</span>
                  <span className="flex items-center gap-1"><ClockIcon className="w-3 h-3 text-slate-500" />{article.readTime} دقيقة</span>
                </div>
                <h3 className="text-base font-bold text-slate-900 group-hover:text-sky-700 transition-colors leading-snug"><Link to={`/articles/${article.slug}`}>{article.title}</Link></h3>
              ...
              <div className="pt-4 mt-4 border-t border-slate-200/80 flex items-center justify-between text-xs">
                <span className="flex items-center gap-1 text-slate-500 text-[11px]"><UserIcon className="w-3.5 h-3.5 text-sky-400" />دكتور هيثم الخطيب</span>
                <Link to={`/articles/${article.slug}`} className="text-blue-700 group-hover:text-blue-800 font-bold flex items-center gap-1 text-xs"><span>اقرأ المقال</span><ChevronLeftIcon className="w-3.5 h-3.5" /></Link>
              </div>
            </article>
          ))}
        </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="my-8 p-5 bg-sky-50/40 border border-sky-200 rounded-xl text-xs text-sky-800 shadow-sm">
          <div className="flex items-center gap-2 mb-3 text-sky-700 font-bold text-sm"><ShieldCheckIcon className="w-4 h-4 text-emerald-400" /><span>مراجع رسمية وتثقيفية</span></div>
          <ul className="space-y-2.5">
            <li className="flex items-start gap-2"><ExternalLinkIcon className="w-3.5 h-3.5 text-sky-400 shrink-0 mt-0.5" /><div><a href="https://www.moh.gov.sa/" target="_blank" rel="noopener noreferrer" className="font-semibold text-blue-700 hover:underline">وزارة الصحة السعودية</a><p className="text-slate-600 text-[11px] mt-0.5 leading-snug">المصدر الرسمي للمعلومات والخدمات الصحية في المملكة.</p></div></li>
            <li className="flex items-start gap-2"><ExternalLinkIcon className="w-3.5 h-3.5 text-sky-400 shrink-0 mt-0.5" /><div><a href="https://www.sfda.gov.sa/" target="_blank" rel="noopener noreferrer" className="font-semibold text-blue-700 hover:underline">الهيئة العامة للغذاء والدواء</a><p className="text-slate-600 text-[11px] mt-0.5 leading-snug">مرجع تنظيمي وسلامة دوائية رسمي.</p></div></li>
            <li className="flex items-start gap-2"><ExternalLinkIcon className="w-3.5 h-3.5 text-sky-400 shrink-0 mt-0.5" /><div><a href="https://www.who.int/" target="_blank" rel="noopener noreferrer" className="font-semibold text-blue-700 hover:underline">منظمة الصحة العالمية</a><p className="text-slate-600 text-[11px] mt-0.5 leading-snug">إرشادات ومراجع دولية في الصحة الإنجابية والرعاية الصحية.</p></div></li>
          </ul>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        <div className="flex items-center gap-2 border-b border-slate-200 pb-3"><CircleQuestionMarkIcon className="w-6 h-6 text-amber-400" /><h2 className="text-2xl font-bold text-slate-900">الأسئلة الشائعة والإجابات الطبية</h2></div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {FAQS.map((f, i) => <div key={i} className="bg-white border border-slate-200 rounded-xl p-6 space-y-2"><h3 className="font-bold text-slate-900 text-base flex items-start gap-2"><span className="text-amber-400 shrink-0 font-extrabold">س:</span><span>{f.q}</span></h3><p className="text-xs text-slate-600 leading-relaxed pr-5">{f.a}</p></div>)}
        </div>
      </section>

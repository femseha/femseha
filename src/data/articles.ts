...
const allArticles = [
  ...
  ...(seoContentBatch07 as ArticleRecord[]),
  ...(seoContentBatch08 as ArticleRecord[]),
  ...(seoContentBatch09 as ArticleRecord[]),
  ...(seoContentBatch10 as ArticleRecord[]),
];
...
export const articles: ArticleRecord[] = uniqueArticles.map((article) => {
  ...
  const withPillar = override
    ? {
        ...
        ...(override.contentAppend ? { content: `${withFaq.content}\n\n${override.contentAppend}` } : {}),
      }
    : withFaq;
  const withLegacy = legacyOverride
    ? {
        ...
        ...(legacyOverride.primaryKeyword ? { primaryKeyword: legacyOverride.primaryKeyword } : {}),
        ...(legacyOverride.contentReplace ? { content: legacyOverride.contentReplace } : {}),
        ...(legacyOverride.faqReplace ? { faq: legacyOverride.faqReplace } : {}),
        ...(legacyOverride.relatedReplace ? { related: legacyOverride.relatedReplace } : {}),
      }
    : withPillar;
